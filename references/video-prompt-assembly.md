...
### 第 3 层：约束层（锚点）
```
约束：角色面部/服装/武器参考@图1不可变。画面锚点参考@图3-5不偏离。运镜/景别/焦段/色彩/灯光/转场/时长严格按@图2故事板执行。[如有台词] 台词：帧 N 口型同步「[内容]」[表演要求]。字幕：帧 N 底部中央「[内容]」。
```
...
```
生成 15s 电影级视频，主题《一剑开天》。参考图片：@图1（角色设定卡）@图2（故事板）@图3（蓄力竖举）@图4（劈天裂空）@图5（身后收剑台词）。
...
约束：角色面部/服装/武器参考@图1不可变。画面锚点参考@图3-5不偏离。运镜/景别/焦段/色彩/灯光/转场/时长严格按@图2故事板执行。台词：帧7口型同步「所有人感受。」沉稳语气不眯眼。字幕：帧7底部中央「所有人感受。」

风格：VS5 暴力美学。色彩弧线：暗蓝灰#2a3a4a→冷灰#4a5568→暗金#c9a84c→纯黑#0a0a0a→暖金#d4af37→暗调#1a1a1a。
cinematic 15s, follow @图2 storyboard timeline, character via @图1, anchors via @图3-5, 8K, no watermark, no broken faces.
```
