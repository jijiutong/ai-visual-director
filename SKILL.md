...
▸ 辅助锚点 1：场景参考图 — 防止场景跑偏（环境/光线/天空/氛围跳变）
  → 回复"生成场景参考图"或"场景锚点"，可选以下 7 种方法（可叠加）：
    ...
    · GPT Image 2 镜头角度修改 — 基于一张场景图直接出不同角度的分镜图
      → 回复"镜头角度分镜"或"camera remix"
      → 步骤：生成场景参考图 → 逐镜从不同角度「拍摄」→ N 张一致场景的分镜图
      → 每镜 prompt 加约束：「同一场景，仅改变拍摄位置和镜头角度，禁止改变场景布局」
...
- **生成场景参考图 / 场景锚点** → 7 种方法可选（俯拍图/4宫格/9宫格/单张全景/全能参考/720全景/环绕截图/GPT镜头角度修改），详见场景一致性参考
- **全能参考图 / all-in-one** → 一张图锁定场景+角色+光照+道具，最高效锚点方案
- **生成全景图 / 720全景** → 360°×180° 全景环境图，覆盖所有拍摄方向
- **环绕截图 / 视频截图法** → 生成环绕视频→截取8角度关键帧→拼接参考图
- **镜头角度分镜 / camera remix** → 用 GPT Image 2 基于场景图从不同角度生成分镜图，100%场景一致
