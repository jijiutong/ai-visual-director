...
▸ 辅助锚点 1：场景参考图 — 防止场景跑偏（环境/光线/天空/氛围跳变）
  → 回复"生成场景参考图"或"场景锚点"
  → 生成一张环境锚点图：固定场景地点+天气+光线+时间+环境关键元素
  → 作用：确保视频中背景/天空/光线/阴影/环境细节跨帧一致不变

▸ 辅助锚点 2：角色设定卡 — 防止角色跑偏
  → 回复"生成角色卡"或"格式3"
  → 作用：确保视频中角色面部/服装/武器不变形

▸ 辅助锚点 3：视频分镜图（3-4 张）— 防止画面跑偏
  → 回复"生成视频分镜图"或"合并帧"
...
【Seedance 视频生成流程】
1. 准备素材：故事板（主参考）+ 场景参考图（场景锚点）+ 角色卡（角色锚点）+ 视频分镜图 3-4 张（画面锚点）
2. 生成视频 prompt（回复"出视频 prompt"）— AI 自动整合故事板全序列 + 所有锚点约束，生成一条完整视频 prompt
3. 将 prompt + 4-5 张参考图（场景参考图+角色卡+故事板+分镜图）输入 Seedance
...
- **生成场景参考图 / 场景锚点** → 输出一张环境锚点图，锁定场景地点+天气+光线+时间+环境关键元素，确保视频中背景/天空/光影跨帧一致不变
